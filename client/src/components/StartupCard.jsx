...
export default function StartupCard({ startup, to, onExpressInterest }) {
  ...
  const designated =
    isDesignated(startup.status) || startup.status === "designated";
  const name =
    startup.legalName ||
    startup.companyName ||
    startup.name ||
    ...
    <div className="group bg-white rounded-3xl border border-slate-200/90 p-6 shadow-sm hover:shadow-xl hover:border-teal-400/60 transition-all duration-300 flex flex-col justify-between relative overflow-hidden">
      ...
      <div>
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-3">
            <div
              className={`w-12 h-12 rounded-2xl border flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform ${sectorStyle.iconBg}`}
            >
              {startup.logo ? (
                <img
                  src={startup.logo}
            ...
            <div>
              <span
                className={`inline-flex items-center px-2.5 py-0.5 rounded-lg text-[11px] font-bold border ${sectorStyle.bg}`}
              >
                {sector}
              </span>
      ...
      <div className="mt-5 pt-3 border-t border-slate-100 flex items-center justify-between gap-2 text-xs">
        ...
        <div className="flex items-center gap-2">
          {onExpressInterest && designated && (
            <button
              type="button"
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                onExpressInterest(startup);
              }}
              className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-teal-50 text-teal-800 font-bold border border-teal-200 hover:bg-teal-100 transition-colors"
            >
