...
async function canAccessDocuments(user, startupId) {
  ...
  if (!startup)
    return { allowed: false, startup: null, reason: "Startup not found" };
  ...
  if (user.role === "investor") {
    // Check NEW connection pipeline first (Proclamation 1396/2025 system)
    const connection = startup.investorConnections.find(
      (c) =>
        c.investor.toString() === user._id.toString() && c.dataRoomApproved,
    );
    if (connection) {
      return { allowed: true, startup, role: "investor" };
    }

    // Fallback: check legacy access request (v1 system)
...
async function fetchCloudFile(doc) {
  ...
  if (doc.cloudinaryUrl) urlsToTry.push(doc.cloudinaryUrl);
  const resourceType = doc.resourceType || getResourceType(doc.mimeType);
  urlsToTry.push(
...
exports.uploadDocument = async (req, res) => {
  try {
    ...
    if (!startup) {
      return res
        .status(404)
        .json({ success: false, message: "You do not have a startup yet" });
    }
...
exports.getMyDocuments = async (req, res) => {
  try {
    ...
    const docs = await Document.find({ startup: startup._id }).sort({
      createdAt: -1,
    });
...
exports.getStartupDocuments = async (req, res) => {
  try {
    ...
    const docs = await Document.find({ startup: startupId }).sort({
      createdAt: -1,
    });
    res.status(200).json({ success: true, count: docs.length, data: docs });
  } catch (error) {
...
exports.downloadDocument = async (req, res) => {
  try {
    ...
    if (!access.allowed) {
      return res
        .status(403)
        .json({ success: false, message: access.reason || "Not authorized" });
    }
